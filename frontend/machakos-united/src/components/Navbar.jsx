  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      scrolled
        ...
        : 'bg-black/20 backdrop-blur-sm'                              // top of page: transparent
    }`}>
      ...
      <div className={`md:hidden transition-all duration-300 overflow-hidden ${
        ...
        <div className="bg-gray-900/98 backdrop-blur-sm px-4 py-4 flex flex-col gap-4
          ...
          {user?.is_admin_user && (
            <Link to="/admin"
                  className="text-xs text-green-400 uppercase tracking-widest font-medium
                            border border-green-500/20 px-3 py-1.5 rounded-full w-fit">
              Admin Panel
            </Link>
          )}
          ...
          <div className="border-t border-gray-800 pt-4 flex flex-col gap-3">
            {user ? (
              ...
              // ── logged out state ──────────────────────────────────────────
              <>
                <Link to="/login" className="text-sm text-gray-300 font-medium">
                  Sign in
                </Link>
                <Link to="/register"
                      className="text-sm bg-green-600 text-white font-medium px-4 py-2
